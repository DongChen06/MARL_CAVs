...
class AbstractEnv(gym.Env):
    ...
    def safety_supervisor(self, actions):
        ...
        for i, vehicle_and_action in enumerate(vehicles_and_actions):
            ...
            # vehicle is on the ramp
                ...
                v_fr, v_rr = env_copy.road.surrounding_vehicles(vehicle)
                if len(env_copy.road.network.side_lanes(vehicle.lane_index)) != 0:
                    v_fl, v_rl = env_copy.road.surrounding_vehicles(vehicle, env_copy.road.network.side_lanes(
                                                                        vehicle.lane_index)[0])
            ...
            for t in range(n_points):
                ...
                for v in [v_fl, v_fr, vehicle, v_rl, v_rr]:
                    ...
                    if len(v.trajectories) == n_points and t == 0 and v is not vehicle:
                        pass
                    # other surrounding vehicles, IDM or MDP
                    else:
                        if type(v) is IDMVehicle:
                            ...
                            # assume we know the actions of other ego vehicles
                ...
                # check collision for each time step TODO: Check
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        """
        self.vehicle_posx.append(([v.position[0] for v in self.road.vehicles]))
        self.vehicle_posy.append(([v.position[1] for v in self.road.vehicles]))
        self.vehicle_speed.append([v.speed for v in self.road.vehicles])

        state = []
        for v in self.road.vehicles:
            # is CAV, x, y, vx, vy
            state.append([type(v) is MDPVehicle, v.position[0], v.position[1], v.velocity[0], v.velocity[1]])
        ...
        obs = self.observation_type.observe()
        reward = self._reward(action)  # TODO: action or self.new_action
        ...
        self.new_actions.append(self.new_action)

        info = {"speed": self.vehicle.speed,
            "crashed": self.vehicle.crashed,
            "actions": np.array(self.actions),
            "new_actions": np.array(self.new_actions),
            "action_mask": np.array(available_actions),
            "average_speed": average_speed,
            "vehicle_speed": np.array(self.vehicle_speed),
            "vehicle_positionx": np.array(self.vehicle_posx),
            "vehicle_positiony": np.array(self.vehicle_posy)}
